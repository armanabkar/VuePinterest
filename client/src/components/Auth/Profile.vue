<template>
  <v-container class="text-xs-center">
    ...
    <v-flex sm6 offset-sm3>
      <v-card class="userCard" color="white">
        <v-layout>
          <v-flex xs5>
            <v-card-media
              height="125px"
          ...
          <v-flex xs7>
            <v-card-title primary-title>
              <div>
                <div class="headline text-uppercase">{{ user.username }}</div>
    ...
    <v-container v-if="!userFavorites.length">
      <v-layout row wrap>
        <v-flex xs12>
          <h3>You have no favorites currently. Go and add some!</h3>
        </v-flex>
      </v-layout>
    </v-container>
    ...
    <v-container class="mt-3" v-else>
      ...
      <v-layout row wrap>
        <v-flex xs12 lg4 sm6 v-for="favorite in userFavorites" :key="favorite._id">
          <v-card class="mt-3 ml-1 mr-2 userCard" hover>
            <v-card-media
    ...
    <v-container v-if="!userPosts.length">
      <v-layout row wrap>
        <v-flex xs12>
          <h3>You have no posts currently. Go and add some!</h3>
        </v-flex>
      </v-layout>
    </v-container>
    ...
    <v-container class="mt-3" v-else>
      <v-flex xs12>
        <h2 class="font-weight-medium">
          Your Posts
          <span class="font-weight-regular userCard">({{ userPosts.length }})</span>
        </h2>
      </v-flex>
      <v-layout row wrap>
        <v-flex xs12 sm6 lg4 v-for="post in userPosts" :key="post._id">
          <v-card class="ml-1 mr-2 userCard" hover>
            <v-card-media
              @click="goToPost(post._id)"
              height="25vh"
              :src="post.imageUrl"
            ></v-card-media>
            <v-card-text class="title">{{ post.title }}</v-card-text>
